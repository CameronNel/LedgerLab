...
export function openCommandPalette(host: HTMLElement, index: readonly SearchResult[], activate: (result: SearchResult) => void): () => void {
    ...
    dialog.innerHTML = `<header><h2>Find your work</h2><button type="button" data-search-close aria-label="Close search">×</button></header><label for="ledgerlab-global-query">Search apps, assignments, documents, journals, accounts and lessons</label><input id="ledgerlab-global-query" type="search" role="combobox" aria-autocomplete="list" aria-expanded="true" aria-controls="ledgerlab-search-options" autocomplete="off" spellcheck="false" placeholder="Try an invoice reference, supplier or account" maxlength="200"><p class="pc-search-count" role="status" aria-live="polite"></p><div id="ledgerlab-search-options" class="pc-search-results" role="listbox" aria-label="Search results"></div><footer>Arrow keys select a result. Enter opens it. Escape returns to your work.</footer>`;
    host.append(dialog);
    ...
    const highlight = () => { if(results.length) input.setAttribute('aria-activedescendant','ledgerlab-result-'+selected); else input.removeAttribute('aria-activedescendant'); list.querySelectorAll<HTMLButtonElement>('button').forEach((button, i) => { button.dataset.selected = String(i === selected); button.setAttribute('aria-selected',String(i === selected)); }); list.querySelector('[data-selected="true"]')?.scrollIntoView({ block: 'nearest' }); };
    const render = () => {
        results = searchWorkspace(index, input.value);
        selected = 0;
        list.innerHTML = results.length ? results.map((result, i) => `<div role="presentation"><button type="button" role="option" tabindex="-1" id="ledgerlab-result-${i}" data-search-result="${i}"><span class="pc-search-kind">${escape(result.kind)}</span><span><strong>${escape(result.title)}</strong><small>${escape(result.detail)}</small></span></button></div>`).join('') : '<p class="pc-search-empty">No matches in your available work. Try a shorter reference or a different word.</p>';
