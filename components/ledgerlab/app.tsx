...
const NAV=[
 {group:'WORKSPACE',items:[{id:'desktop',title:'Finance PC',icon:Monitor},{id:'career',title:'Your finance desk',icon:BookOpen},{id:'overview',title:'Overview',icon:LayoutDashboard},{id:'practice',title:'Practice path',icon:GraduationCap},{id:'documents',title:'Source documents',icon:Files}]},
 {group:'BOOKKEEPING',items:[{id:'ledger',title:'Journal & ledger',icon:BookOpen},{id:'receivables',title:'Receivables',icon:ReceiptText},{id:'payables',title:'Payables',icon:WalletCards},{id:'bank',title:'Bank reconciliation',icon:Landmark},{id:'payroll',title:'Payroll',icon:Users}]},
 {group:'MONTH-END & REPORTING',items:[{id:'close',title:'Month-end close',icon:ListChecks},{id:'assets',title:'Assets & leases',icon:Building2},{id:'inventory',title:'Inventory',icon:Package},{id:'provisions',title:'Provisions',icon:Scale},{id:'tax',title:'Tax workspace',icon:Calculator},{id:'reports',title:'Financial statements',icon:ChartNoAxesCombined},{id:'audit',title:'Audit support',icon:ShieldCheck}]},
 {group:'YOUR TOOLKIT',items:[{id:'knowledge',title:'Accounting desk',icon:GraduationCap},{id:'settings',title:'Case, notebook & data',icon:Settings}]},
];
const validViews=new Set(NAV.flatMap(n=>n.items.map(i=>i.id)));
function Navigation({view,go,complete,total,period}:{view:string;go:(v:string)=>void;complete:number;total:number;period:string}){const {isMobile,setOpenMobile}=useSidebar();return <Sidebar className="app-sidebar" collapsible="offcanvas"><SidebarHeader className="p-0"><div className="brand-lockup"><div className="brand-mark"><BookOpen size={23} strokeWidth={1.7}/></div><div className="brand-word">Ledger<span>Lab</span></div></div><div className="company-switch"><span className="company-monogram">H&C</span><div><strong>Harbour & Co.</strong><small>Accounting practice company</small></div></div></SidebarHeader><SidebarContent>{NAV.map(group=><SidebarGroup key={group.group}><SidebarGroupLabel>{group.group}</SidebarGroupLabel><SidebarMenu>{group.items.map(item=><SidebarMenuItem key={item.id}><SidebarMenuButton isActive={view===item.id} onClick={()=>{go(item.id);if(isMobile)setOpenMobile(false);}}><item.icon/><span>{item.title}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroup>)}</SidebarContent><SidebarFooter className="p-0"><div className="sidebar-bottom"><strong>{period}</strong><Completion done={complete} total={total}/><span>Build skill through doing.</span></div></SidebarFooter></Sidebar>;}
