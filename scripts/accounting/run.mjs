...
const mode=process.argv[2]??'check',entries={workday:'workday-check.ts',review:'month-review-check.ts',check:'check.ts',export:'export-case.ts',workflows:'workflow-check.ts',persistence:'persistence-check.ts',career:'career-check.ts',desktop:'desktop-check.ts',render:'render-check.tsx'};
if(!entries[mode])throw new Error('Use workday, review, check, workflows, persistence, career, desktop, render or export.');
