...
const css=['desktop.css','workspace.css','responsive.css'].map(name=>readFileSync(resolve(root,'components/ledgerlab/desktop',name),'utf8')).join('\n');
const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>LedgerLab · Finance Workstation v3.2 · Day-by-Day</title><style>html,body{margin:0;height:100%;overflow:hidden}#preview-label{position:fixed;bottom:65px;right:14px;font:10px Arial;color:#c6e1df;z-index:30;pointer-events:none}${css}</style></head><body><div id="desktop"></div><div id="preview-label">FICTIONAL TRAINING COMPANY · AUD 2025</div><script>${bundle.replace(/<\/script/gi,'<\\/script')}</script></body></html>`;
